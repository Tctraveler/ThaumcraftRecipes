...
public class ItemTransmutationFocus extends ItemFocusBasic {
    ...
    public AspectList getVisCost(ItemStack focus) {
        return new AspectList().add(Aspect.ORDER,500).add(Aspect.WATER,500).add(Aspect.EARTH,500).add(Aspect.FIRE,500).add(Aspect.AIR,500).add(Aspect.ENTROPY,500);
    }
    @Override
    ...
    public ItemStack onFocusRightClick(ItemStack wandstack, World world, EntityPlayer player, MovingObjectPosition movingobjectposition) {
        ...
        if((world.getBlock(x,y,z)== Blocks.stone||world.getBlock(x,y,z)==Blocks.cobblestone)&&wandCasting.consumeAllVis(wandstack,player,this.getVisCost(wandstack),true,false)){
